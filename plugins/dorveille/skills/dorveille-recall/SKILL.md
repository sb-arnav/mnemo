...
the agent mid-conversation. dorveille-recall makes them searchable, so you can pull
...
- The index is local at `~/.dorveille/sessions.db` and refreshes incrementally; if a
  very recent session is missing, run `dorveille recall index`.
