...
MODEL="${DORVEILLE_VERIFY_MODEL:-${DORVEILLE_REVIEW_MODEL:-claude-sonnet-4-6}}"
LOG="$DORVEILLE_HOME/logs/verify.log"
...
UNV="$(DORVEILLE_HOME="$DORVEILLE_HOME" python3 "$LESSON" unverified 2>/dev/null)"
...
timeout "${DORVEILLE_VERIFY_TIMEOUT:-240}" env \
