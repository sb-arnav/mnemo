...
# happen (before the fix), into dorveille's own corrections feed. A user
...
[ -n "${DORVEILLE_REVIEWING:-}" ] && exit 0     # don't capture the worker's own prompt
...
if printf '%s' "$PROMPT" | grep -qiE \
  ...
  DORVEILLE_HOME="$DORVEILLE_HOME" python3 - "$PROMPT" <<'PY' 2>/dev/null || true
import json, os, sys
from datetime import datetime, timezone
home = os.environ.get("DORVEILLE_HOME", os.path.expanduser("~/.dorveille"))
