...
#
#   sweep.sh                 review up to DORVEILLE_SWEEP_MAX idle unreviewed sessions (cron)
...
PROJECTS="${DORVEILLE_PROJECTS_DIR:-$HOME/.claude/projects}"
LOG="$DORVEILLE_HOME/logs/sweep.log"
MARKERS="$DORVEILLE_HOME/reviewed"
mkdir -p "$DORVEILLE_HOME/logs" "$MARKERS"

[ -f "$DORVEILLE_HOME/OFF" ] && exit 0
[ -n "${DORVEILLE_REVIEWING:-}" ] && exit 0

MIN_TURNS="${DORVEILLE_SWEEP_MIN_TURNS:-4}"     # skip trivial sessions
IDLE_MIN="${DORVEILLE_SWEEP_IDLE_MIN:-15}"      # don't touch sessions edited in last N min (likely active)
MAX="${DORVEILLE_SWEEP_MAX:-3}"                 # cap reviews per run (each is a claude -p call)
