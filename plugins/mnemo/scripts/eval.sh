...
#
# Run on a schedule (cron) or on demand via `mnemo eval`.
set -uo pipefail

PLUGIN_ROOT="${CLAUDE_PLUGIN_ROOT:-$(cd "$(dirname "$0")/.." && pwd)}"
MNEMO_HOME="${MNEMO_HOME:-$HOME/.mnemo}"
MODEL="${MNEMO_EVAL_MODEL:-${MNEMO_REVIEW_MODEL:-claude-sonnet-4-6}}"
LOG="$MNEMO_HOME/logs/eval.log"
...
PROBES="$(MNEMO_HOME="$MNEMO_HOME" python3 "$LESSON" probes 2>/dev/null)"
...
timeout "${MNEMO_EVAL_TIMEOUT:-240}" env \
