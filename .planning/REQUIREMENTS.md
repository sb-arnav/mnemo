# dorveille — v1 Requirements (the trust layer)
...
- [ ] **VERIFY-02**: A lesson that fails verification is moved to `~/.dorveille/quarantine/` with its failing reason recorded — never silently dropped — and is reviewable.
- [ ] **VERIFY-03**: The verifier runs on the user's subscription via the real `claude` binary (non-bare `-p`), recursion-guarded by `DORVEILLE_REVIEWING`, with a bounded timeout.
- [ ] **VERIFY-04**: A human can release (`dorveille quarantine release <id>`) or discard a quarantined lesson; the action is auditable.
...
- [ ] **EVAL-03**: `dorveille status` surfaces a reproducible "did learning help?" signal — probe pass-rate over time + repeat-correction trend.
...
- [ ] **NATIVE-02**: The `dorveille` CLI exposes the trust surface: `dorveille quarantine [list|release|discard]`, `dorveille trust [list]`, `dorveille eval [run]`, alongside existing `status`.

## v2 / Deferred

- [ ] **PORT-01**: export/import learned lessons across repos / a team (`dorveille export` / `import`).
